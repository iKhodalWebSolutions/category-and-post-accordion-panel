<?php if ( ! defined( 'ABSPATH' ) ) exit; 
	...
	$final_width = $params["apcp_image_content_width"];  	 
	$apcp_image_height = $params["apcp_image_height"]; 
	$apcp_image_content_width = $params["apcp_image_content_width"];  	
	$apcp_mouse_hover_effect = $params["apcp_mouse_hover_effect"];   

	if( $this->apcp_getTotalPosts( $category_id, $post_search_text, 0, $is_default_category_with_hidden ) > 0 ) {
		...
		if(  !( sanitize_text_field( $params["hide_searchbox"] ) == 'yes' ) ) { 
			...
			<div class="ik-post-category"> 
				...
				<span class="ik-search-button <?php echo $_m_bottom; ?>" onclick='apcp_fillPosts( "<?php echo esc_js( $params["vcode"]."-".$this->cat_replace_dash($category_id) ); ?>", "<?php echo esc_js($category_id); ?>", request_obj_<?php echo esc_js( $params["vcode"] ); ?>, 2)'>  <img width="18px" alt="Search" height="18px" src="<?php echo apcp_media.'images/searchicon.png'; ?>" />
				</span>
				<div class="clrb"></div>
			</div>
		 <?php
	...
	$_total_posts = $this->apcp_getTotalPosts( $category_id, $post_search_text, 1, $is_default_category_with_hidden );
	if( $_total_posts <= 0 ) {
		...
		<div   class='ikh-post-item-box  pid-<?php echo esc_attr( $_post->post_id ); ?>'>
			<div class="ikh-post-item ikh-simple"> 
				...
				if( $params["apcp_hide_post_image"] == "no" ) { ?>
					<div   class='ikh-image' onmouseout="apcp_pr_item_image_mouseout(this)" onmouseover="apcp_pr_item_image_mousehover(this)">
					 ...
					 <div class='ikh-content'>
						 <div class="ikh-content-data">
							...
							<div class='ik-post-name'>
								...
								<?php  
								if( $params["apcp_hide_post_short_content"] == "no" ) { ?>
	...
	if( $params["apcp_hide_paging"] == "no" && $params["apcp_select_paging_type"] == "load_more_option"   && $_total_posts > sanitize_text_field( $params["apcp_number_of_post_display"] ) ) {
	
		?>	
		<div class="clr"></div>
		<div style="display:none" class='ik-post-load-more'  align="center" onclick = 'apcp_loadMorePosts( "<?php echo esc_js( $category_id ); ?>", "<?php echo esc_js( $_limit_start+$_limit_end ); ?>", "<?php echo esc_js( $params["vcode"]."-".$this->cat_replace_dash($category_id) ); ?>", "<?php echo esc_js( $_total_posts ); ?>", request_obj_<?php echo esc_js( $params["vcode"] ); ?> )'>
